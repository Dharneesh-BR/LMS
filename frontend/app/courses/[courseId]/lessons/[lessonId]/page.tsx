...
export default function LessonPage({ params }: { params: { courseId: string; lessonId: string } }) {
  ...
  async function markComplete() {
    const result = await apiFetch<{ progress: LessonProgress }>("/api/progress/update", {
      method: "POST",
      body: JSON.stringify({ courseId: params.courseId, lessonId: params.lessonId, completed: true })
    });
  ...
  function saveVideoProgress(update: VideoProgressUpdate) {
    void apiFetch<{ progress: LessonProgress }>("/api/progress/update", {
      method: "POST",
      body: JSON.stringify({
        ...
        {!lesson ? <p>Loading lesson...</p> : (
          <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
            ...
            <aside className="h-fit rounded-lg border border-mist bg-paper p-5 shadow-card lg:sticky lg:top-24">
              ...
              <button onClick={markComplete} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-md bg-coral px-4 py-3 font-semibold text-white shadow-card transition hover:-translate-y-0.5">
